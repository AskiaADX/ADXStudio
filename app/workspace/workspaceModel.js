...
Workspace.prototype._initWatcher = function _initWatcher () {
  ...
  this._watcher.on('change', function (event, filePath) {
    fs.stat(filePath, function (err) {
      const eventName = (!err) ? 'file-changed' : 'file-removed';
...
Workspace.prototype.toJSON = function toJSON () {
  ...
  for (let i = 0, l = this.tabs.length; i < l; i += 1) {
    ...
    if (tab.adxVersion) {
      jsonTab.adxVersion = tab.adxVersion;
    }

    if (tab.adxType) {
      jsonTab.adxType = tab.adxType;
    }
...
Workspace.prototype.moveTab = function moveTab (tab, targetPane, callback) {
  ...
  const self = this;
  this.find(tab, function (err, tab) {
    if (err) {
      if (typeof callback === 'function') {
...
Workspace.prototype.findAll = function findAll (directoryPath, callback) {
  const results = [];
  const panes = [];
  this.tabs.forEach((tab) => {
    if (tab.path.startsWith(directoryPath)) {
      results.push(tab);
      panes.push(this.panes.mapByTabId[tab.id]);
    }
  });
  callback(null, results, panes);
};
 ...
 * @param {Error} callback.err Error
 */
Workspace.prototype.unwatchTabsIn = function unwatchTabsIn (parentDir) {
  const self = this;
 ...
 * @param {Error} callback.err Error
 */
Workspace.prototype.rewatchTabsIn = function rewatchTabsIn (parentDir) {
  const self = this;
  const lowerCaseDir = parentDir.toLowerCase();
  // Iterate through all tabs
  self.tabs.forEach(function (tab) {
