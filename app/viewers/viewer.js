(function () {
  const matchTabId = /\?tabid=([^&]+)/gi.exec(window.location.href);
  if (!matchTabId || !matchTabId.length) {
    throw new Error('Invalid context, the tab id is not defined');
  }
  const isAltFrame = /&altFrame=1/gi.test(window.location.href);
  const tabId = matchTabId[1];
  const parent = window.parent;
  const workspaceView = parent.workspace;
  const tab = workspaceView.findTab(tabId);
  ...
  const viewer = {
     ...
     */
    changeFontSize: function onChangeFontSize () { /* Function that should be implemented in the viewer */ },
    /**
     * Save content
     * Should be implemented by the viewer
     */
